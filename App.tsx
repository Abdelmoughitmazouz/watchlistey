...
const App = () => {
    ...
    // Internal Navigation Helper: Transforms generic paths to localized paths
    const handleNavigate = (path: string, state?: Show) => {
        // If path is already localized (starts with /es/, /ar/, etc), use as is
        // Otherwise, localize it based on current language
            ...
            if (isSupabaseConfigured) {
                ...
                if (session) {
                    ...
                    if (fullProfile) {
                        ...
                        if (profile) {
                             setUser({ ...profile, list: {}, favorites: {}, characters: {} });
                        }
                    }
                }
            }
    ...
    // Handle Content Resolution and Routing Logic
    useEffect(() => {
        const resolveContent = async () => {
            // Parse the current URL using our routing utility
            const route = parsePath(pathname);
            
            // 1. Sync Language
            if (route.lang !== i18n.language) {
                i18n.changeLanguage(route.lang);
            }

            // 2. Handle Content Routes
            if (route.type && route.slug && ['movie', 'tv'].includes(route.type)) {
                ...
                try {
                    // Check if language changed since last render
                    const languageChanged = prevLanguageRef.current !== route.lang;
                    
                    if (!languageChanged && window.history.state && (window.history.state as Show).id) {
                       show = window.history.state as Show;
                    } 
                    ...
                    if (show) {
                        ...
                        const currentSlugRaw = slug;

                        setExtraShows(prev => new Map(prev).set(`${show!.media_type}-${show!.id}`, show!));
                        setSelectedShow(show);

                        // SEO Friendly URL Redirect (e.g. English slug -> Localized slug)
                        if (newSlug && currentSlugRaw && newSlug !== decodeURIComponent(currentSlugRaw)) {
                            // Construct the correct path with current lang and type
                ...
                if (username) {
                    if (user && user.username === username) {
                        ...
                        if (profile) {
                            ...
                            setViewedProfile({ ...demoUser, username: username, name: username });
                        } else {
                            setViewedProfile(undefined);
                        }
                    }
                }
                setIsContentNotFound(false);
            } else if (
                // Safe check for static and generic routes
                route.type === 'person' ||
                route.type === 'genre' ||
                route.type === 'network' ||
                !route.rest || // Home
                route.rest === 'blog' || 
                route.rest.startsWith('blog/') ||
                route.rest.startsWith('settings') ||
                route.rest.startsWith('lists')
            ) {
                setIsContentNotFound(false);
            }
    ...
    const handleUpdateListStatus = async (showId: number, status: ListStatus | null, show?: Show, customAddedAt?: string) => {
        ...
        // Anime/Manga handling removed, fallback to TV/Movie types from TMDB
        ...
        try {
            if (status === null) {
                 ...
                 else await supabase.from('list_items').delete().match({ user_id: user.id, show_id: showId, media_type: dbMediaType }); // Ensure delete matches media_type
            } else {
                 if (dbMediaType === 'person') {
    ...
    // Determine current route state using robust parser
    const currentRoute = parsePath(pathname);
    
    // Determine if we should show the hero header (transparent)
    // Now relies on resolved type from parsePath, ensuring it works even with localized segments like /ja/映画/...
    const isShowDetail = !!(currentRoute.type && currentRoute.slug && ['movie','tv'].includes(currentRoute.type));
    
    // Complex regex for episode page: /lang/tv/slug/season/X/episode/Y
    // Since parsePath strips language, we check 'rest'
    const isEpisodePage = /\/season\/[^/]+\/episode\/[^/]+$/.test(pathname);
    
    // Home is when rest is empty
    const isHome = !currentRoute.rest;
    
    const hasHero = isHome || isShowDetail || isEpisodePage;

    // Check if we are in the Builder mode
    // We do NOT want to hide the header anymore, per user request.
    ...
    const renderContent = () => {
        ...
        const path = currentRoute.rest; // Work with the language-stripped path
        ...
        if (path === 'lists/new') return <TierListBuilder onNavigate={handleNavigate} />;
        if (path.startsWith('lists/')) {
             // Dynamic route for viewing/editing lists
             return <TierListBuilder onNavigate={handleNavigate} />;
        }
        ...
        if (path.startsWith('blog/')) {
            ...
            return <BlogPost slug={slug} onNavigate={handleNavigate} onBack={() => handleNavigate('/blog')} />;
        }

        if (path.startsWith('u/')) {
             const parts = path.split('/');
             const activeTab = parts[2]; 
             if (!viewedProfile && isSupabaseConfigured) {
        ...
        // Use parsed route for localized person links
        if (currentRoute.type === 'person' && currentRoute.slug) {
            return <PersonPage personId={currentRoute.slug} onNavigate={handleNavigate} onBack={() => window.history.back()} userList={user?.list || {}} userFavorites={user?.favorites} userCharacters={user?.characters} handleUpdateListStatus={handleUpdateListStatus} handleToggleFavorite={handleToggleFavorite} />;
        }

        // Use parsed route for localized genre links
        if (currentRoute.type === 'genre' && currentRoute.slug) {
            return <GenrePage genreId={currentRoute.slug} onNavigate={handleNavigate} onBack={() => window.history.back()} userList={user?.list || {}} userFavorites={user?.favorites} handleUpdateListStatus={handleUpdateListStatus} handleToggleFavorite={handleToggleFavorite} />;
        }

        // Use parsed route for localized network links
        if (currentRoute.type === 'network' && currentRoute.slug) {
            return <NetworkPage networkId={currentRoute.slug} onNavigate={handleNavigate} onBack={() => window.history.back()} userList={user?.list || {}} userFavorites={user?.favorites} userCharacters={user?.characters} handleUpdateListStatus={handleUpdateListStatus} handleToggleFavorite={handleToggleFavorite} />;
        }

        // Episode Page (Regex on raw path is complex due to localized prefixes, relying on internal navigation logic usually or strict structure)
        // If we arrived here, we probably stripped the lang prefix.
        // Internal URL structure: /tv/:slug/season/:s/episode/:e
        const episodeMatch = path.match(/^tv\/([^/]+)\/season\/([^/]+)\/episode\/([^/]+)$/);
        if (episodeMatch) {
            return <EpisodePage showSlug={episodeMatch[1]} seasonNumber={episodeMatch[2]} episodeNumber={episodeMatch[3]} onBack={() => window.history.back()} onNavigate={handleNavigate} userList={user?.list || {}} handleUpdateListStatus={handleUpdateListStatus} />;
        }

        if (path.endsWith('/cast')) {
            if (selectedShow) return <CastPage show={selectedShow} onBack={() => window.history.back()} onNavigate={handleNavigate} userCharacters={user?.characters} handleUpdateListStatus={handleUpdateListStatus} />;
        }

        // Content Detail Pages
        if (selectedShow && (currentRoute.type && currentRoute.slug)) {
            return <ShowDetail show={selectedShow} allShows={[...movies, ...tvShows]} onBack={() => handleNavigate('/')} onNavigate={handleNavigate} onUpdateShow={setSelectedShow} userList={user?.list || {}} userFavorites={user?.favorites} userCharacters={user?.characters} handleUpdateListStatus={handleUpdateListStatus} handleToggleFavorite={handleToggleFavorite} currentUser={user} notifications={notifications} />;
        }
