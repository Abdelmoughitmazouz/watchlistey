...
-- AUTOMATION TRIGGER: When list_items is updated, log an activity
CREATE OR REPLACE FUNCTION handle_list_activity()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    ...
    IF (TG_OP = 'INSERT' OR OLD.status IS DISTINCT FROM NEW.status) THEN
        ...
        IF activity_action IS NOT NULL THEN
            INSERT INTO user_activities (user_id, show_id, media_type, action, metadata)
            VALUES (NEW.user_id, NEW.show_id, NEW.media_type, activity_action, 
                jsonb_build_object('title', NEW.title, 'image', NEW.poster_path));
        END IF;
    END IF;

    -- 2. Progress Updates
    IF (TG_OP = 'UPDATE' AND OLD.progress IS DISTINCT FROM NEW.progress AND NEW.progress > 0) THEN
        INSERT INTO user_activities (user_id, show_id, media_type, action, metadata)
        VALUES (NEW.user_id, NEW.show_id, NEW.media_type, 'progress_updated', 
            jsonb_build_object('progress', NEW.progress, 'prev_progress', OLD.progress, 'title', NEW.title, 'image', NEW.poster_path));
    END IF;
...
FOR EACH ROW EXECUTE PROCEDURE handle_list_activity();

-- 4. Favorites Table (Separate from Watchlist)
...
-- 5. Characters Table (Favorite People)
...
-- 8. Notifications Table (The INBOX for alerts)
...
-- 9. Subscriptions Table (Active Tracked Shows)
...
-- 10. Episode Tracking Table (Granular tracking)
...
-- 11. Tier Lists Table (Interactive Builder)
